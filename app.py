...
def get_model():
    ...
    if not api_key or api_key.strip() == "" or "YOUR_API_KEY_HERE" in api_key:
        print(f"DEBUG: MISTRAL_API_KEY missing or placeholder in {ENV_PATH}")
        return None
...
def get_youtube_transcript(url):
    ...
    try:
        # ✅ Use fetch() with instance (your version supports this)
        transcript = YouTubeTranscriptApi().fetch(video_id)
        text = " ".join([t.text for t in transcript])
        return text, None
    except Exception as e:
        # Fallback: audio transcription if captions unavailable
        try:
...
def summarize():
    ...
    model = get_model()
    if not model:
        return jsonify({'error': f'Mistral API Key not found. Looking for .env at: {ENV_PATH}'}), 500
